...
function ListagemServicos() {
  ...
  const fetchServicos = async () => {
    try {
      ...
      const response = await axios.get(
        "https://integrador-backend-74994e883ac3.herokuapp.com/listagem/servico",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const updateServico = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.put(`https://integrador-backend-74994e883ac3.herokuapp.com/update/servico/${id}`, {
        clientecnpj: cliente,
        tipo_id: tipoServico
      }, {
        headers: {
